...
pub fn calculate<'a, T: Primitive>(kmean: &KMeans<T>, state: &mut KMeansState<T>, rnd: &'a mut dyn RngCore)
...
mod tests {
    ...
    fn init_kmeanplusplus<T: Primitive>(b: &mut Bencher) where [T;LANES] : SimdArray, Simd<[T;LANES]>: SimdWrapper<T> {
        ...
        b.iter(|| {
            ...
            KMeans::init_kmeanplusplus(&kmean, &mut state, &mut thread_rng());
            state
        });
    }
}